...
mandelbrot_params m_params = {
    .focal_x = 0.0, .focal_y = 0.0, .zoom = 1.0, .max_iterations = 450};
...
void rendering_setup(int screenWidth, int screenHeight) {
  free(mandel_data);
  mandel_data =
      malloc(screenHeight * screenWidth * max_mandel_rows * sizeof(double));
...
TextUI new_text_ui(char *fmt_string, int x, int y) {
  TextUI txt_ui = {.format_str = fmt_string, .x = x, .y = y};
...
void draw_text() {
  ...
  __format_text_ui(&focal_point_text, m_params.focal_x, m_params.focal_y);
  __format_text_ui(&zoom_text, m_params.zoom);

  __format_text_ui(&process_text, current_resolution, p_buffer.progress);
